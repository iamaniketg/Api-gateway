        ...
        stage('Deploy to GKE') {
            steps {
                withCredentials([file(credentialsId: 'gcp-service-account', variable: 'GOOGLE_APPLICATION_CREDENTIALS')]) {
                    script {
                        def fullImage = "${IMAGE_NAME}:${IMAGE_TAG}"
                        def deploymentExists = sh(script: """
                            export PATH=${WORKSPACE}/google-cloud-sdk/bin:\$PATH
                            kubectl get deployment ${K8S_DEPLOYMENT} -n ${K8S_NAMESPACE} --output=jsonpath='{.metadata.name}' 2>/dev/null
                        ...
                        if (deploymentExists) {
                            sh "kubectl set image deployment/${K8S_DEPLOYMENT} ${K8S_DEPLOYMENT}=${fullImage} -n ${K8S_NAMESPACE}"
                        } else {
                            sh "sed -i 's|image: .*|image: ${fullImage}|g' gateway-deployment.yaml"
                        }

                        sh """
                            kubectl apply -f gateway-deployment.yaml -n ${K8S_NAMESPACE}
                            kubectl rollout status deployment/${K8S_DEPLOYMENT} -n ${K8S_NAMESPACE} --timeout=5m
                        """
                    }
                }
